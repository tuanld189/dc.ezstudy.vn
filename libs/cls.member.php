...
class CLS_USER{
	...
	public function getListGroup($where=''){
		$sql="SELECT * FROM `tbl_member_group` WHERE 1=1 ".$where;
		$this->objmysql->Query($sql);
	}
	...
	function Add_new(){
		...
		$pass=hash('sha512',$pass);		
		$pass=md5($pass);
		$sql="INSERT INTO `tbl_member` (`username`,`password`,`firstname`,`lastname`,`birthday`,`gender`,`address`,`phone`,`mobile`,`email`,`joindate`,`gid`,`isactive`) VALUES ";
	...
	public function ChangePass_User($user,$newpass) {
		...
		$pass=hash('sha512',$pass);		
		$pass=md5($pass);
		$sql="UPDATE `tbl_member` SET `password`='".$pass."'";
	...
	function Delete($id){
		$sql="DELETE FROM `tbl_member` WHERE `id` in ('$id')";
		return $this->objmysql->Query($sql);
	}
	...
	public function getGroupUser($par_id,$level=0){
		$sql="SELECT * FROM `tbl_member_group` WHERE par_id=$par_id AND `isactive`=1";
	...
	public function getUserInGroup($gid){
		$sql="SELECT * FROM `tbl_member` WHERE `isactive`=1 AND `gid`=$gid";
	...
	public function getUserID($uid){
		$sql="SELECT * FROM `tbl_member` WHERE `isactive`=1 AND `id`=$uid";
		$objdata=new CLS_MYSQL();
		$objdata->Query($sql);
		if($objdata->Num_rows()){
			echo "<ul class='menu'>";
	...
	function getListCombo($parid,$thisid,$level){
		$sql="SELECT * FROM `tbl_member_group` WHERE `par_id`=$parid AND `isactive`=1";
		$objdata=new CLS_MYSQL();
		$objdata->Query($sql);
		$char="";
		if($level>0){
	...
	public function getCurrentLastId() {
		$sql="SELECT max(id) as max FROM `tbl_member`";
		$objdata=new CLS_MYSQL;
		$objdata->Query($sql);
	...
	public function getNameById($id){
        $objdata=new CLS_MYSQL;
        $sql="SELECT CONCAT(`firstname`,' ',`lastname`) AS 'fullname' FROM `tbl_member`  WHERE isactive=1 AND `user_id` = '$id'";
        $objdata->Query($sql);
        $row=$objdata->Fetch_Assoc();
        return $row['fullname'];
    }
