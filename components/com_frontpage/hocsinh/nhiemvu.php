...
<div class="card-nv skill-section">
	...
	<div class="nav nav-tabs nav-tabs-ctr" role="tablist">
		<a href="#home" role="tab" data-toggle="tab" class="active">Mục tiêu ngày</a>
		<?php if(getInfo('packet')!=''&& getInfo('packet_status')=='running'){?>
		<a href="#about" role="tab" data-toggle="tab">Nhiệm vụ tuần</a>
		<?php }?>
	</div>
				...
				<div class="tab-pane active in" id="home">
					...
					<div class="box-nvngay scrollbar-stype">
						...
						while($v=$item->Fetch_Assoc()) { 
							...
							// die;
							?>
							<div class="item-nvngay <?php echo $class;?>" data-id="<?php echo $v['id'];?>">
					...
					<div class="list box-nv-main scrollbar-stype">
						...
						foreach($_Nhiemvu as $k => $v) { 
							...
							foreach($v as $key=>$item) {
								...
								$i++;
								$title=$item['title']==''? 'Bài tập môn '.$mon:$item['title'];
								...
								<div class=" item-nv <?php if($status==1) echo 'active';?>">
									<a href="" class=" thumb icon <?php echo $ic;?> icon-subject"></a>
									...
									else echo '<a href="'.$url.'" target="_brank" class="btn  btn-action">'.$lambai.' <i class="fa fa-arrow-right"></i></a>';
									echo '</div>';
									?>
								</div>
							<?php } ?>
						...
						<?php }
							...
							echo "<div class='text-center' style='padding: 25px 0;'>Không có nhiệm vụ hôm nay</div>";
						}?>
					</div>
				</div>
				<?php }?>
