...
<head profile="http://www.w3.org/2005/10/profile">
	...
	<link rel="stylesheet" type="text/css" media="all" href="<?php echo EXAM_PATH; ?>global/css/bootstrap.min.css">
	<link rel="stylesheet" type="text/css" media="all" href="<?php echo EXAM_PATH; ?>global/css/style.css?v=1">
	<script src='<?php echo EXAM_PATH;?>global/js/jquery-1.11.2.min.js'></script>
	<script src="<?php echo EXAM_PATH;?>global/js/bootstrap.min.js"></script>
	<script src="https://cdn.ckeditor.com/4.16.1/standard-all/ckeditor.js"></script>
	<script src="<?php echo EXAM_PATH;?>global/js/editorScript.js"></script>
...
$url_full=EXAM_PATH.'lession-exam/'.$exam_id.'.html';
$quiz_index=$page-1;

$reload=isset($_SESSION['EXAM_LIST'][$exam_id])?"yes":"no";
?>
<script>
	...
	$(document).ready(function() {
		...
		if(reload!='no') data=null;
		if(!data){
			_url='<?php echo EXAM_PATH;?>ajaxs/quiz/save.php';
			$.post(_url,{exam_id}, function(rep){ // get exam
				//console.log(rep);
				if(rep==1){
	...
	function getContentPage(data,page, exam_id, quiz_index){
		var _url='<?php echo EXAM_PATH;?>ajaxs/quiz/content.php';
	...
	function getQuiz(exam_id, quiz_index, Quiz_type=1){
		...
		if(data['info_exam']['data_answer'][quiz_id]){
			alert('Câu hỏi đã trả lời!');
			return;
		}			
	...
	function showQuiz(exam_id, quiz_index){
		...
		var isviewQuiz='';
		if(data['iscomplete']=='yes') isviewQuiz=1;
		...
		MathJax.Hub.Queue(["Typeset",MathJax.Hub]);
		checkAnswer(exam_id, quiz_index);
	}
	function checkAnswer(exam_id, quiz_index){
		var data=JSON.parse(localStorage.getItem(name_json));
		if(!data['info_exam']['data_answer']) data['info_exam']['data_answer']=new Object();
		var data_ans=data['info_exam']['data_answer'];
		var item=data[exam_id][quiz_index];
		var quiz_id=item['id'];
		if(item.answers && data_ans[quiz_id]){
			const answers=JSON.parse(item.answers);
			var quizTrue=true;
			for(var i in answers) {
				var istrue=answers[i].is_true;
